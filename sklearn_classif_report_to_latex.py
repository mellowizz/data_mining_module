...
def parse_classification_report(clfreport):
    ...
    assert header.split() == ['precision', 'recall',
                              'f1-score', 'support']
    assert avg_line.split()[0] == 'avg'

    # We cannot simply use split because class names can have spaces.
    # So instead figure the width of the class field by looking at the
    # indentation of the  precision header
...
if __name__ == '__main__':
    with open(sys.argv[1]) as f:
        ...
        try:
            out_obj = sys.argv[2]
            out_obj = open(out_obj)
        except IndexError as e:
            out_obj = sys.stdout
        print(report_to_latex_table(data), file=out_obj)
